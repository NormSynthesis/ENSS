...
public class TaxonomyOfTerms implements Taxonomy {
	...
	public SetOfStrings getRepresentedTerms(String term) {
		...
		this.fillWithRepresentedTerms(term, childTerms);
	 ...
	 * @param childTerms
	 */
	private void fillWithRepresentedTerms(String term, SetOfStrings childTerms) {

		/* If the term does not exist in the taxonomy -> return empty set */
		...
		/* If the term does not represent any term in the taxonomy, 
		 * return a set containing only that term */
		if(edges.isEmpty()) {
			childTerms.add(term);
			return;
		}
		...
		for(TaxonomyEdge edge : this.termsGraph.getInEdges(term)) {
			...
			this.fillWithRepresentedTerms(child, childTerms);
		}		
	}
